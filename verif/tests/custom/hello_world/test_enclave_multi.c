...
// Ajoute/enleve un id d'enclave manuellement 
...
// Csr pour verification solution - pas utilisable sur cette version.
...
// Activation proctection enclave
...
// Set le perf counter pour les miss
...
int main(void)
{   // Set la secu enclave pour faire fonctionner timewarp
    set_enclave_id(1);
    set_secure_flag();
...
static volatile uint32_t line __attribute__((aligned(64))) = 0x12345678;    volatile uint32_t sink = 0;
    ...
    // compteur csr de miss pour verifier hit / miss 
    set_miss_counter();
    //set_lecture_hit();
